...
/**
 */
public class fp_zap_onhit implements OnFireEffectPlugin {
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		float dmg = MathUtils.getRandomNumberInRange(100, weapon.getDamage().getDamage() + 2500f);
                
	
		CombatEntityAPI target = findTarget(projectile, weapon, engine);
		float thickness = dmg/200f;
		float coreWidthMult = dmg/300f;
		Color color = new Color(100,100,220,255); //weapon.getSpec().getGlowColor();
		if (target != null) {
			EmpArcEntityAPI arc = engine.spawnEmpArc(projectile.getSource(), projectile.getLocation(), weapon.getShip(),
					   ...
					   thickness, // thickness
					   color,
					   weapon.getGlowSpriteAPI().getColor()
					   );
			...
			EmpArcEntityAPI arc = engine.spawnEmpArcVisual(from, weapon.getShip(), to, weapon.getShip(), thickness, color, weapon.getGlowSpriteAPI().getColor());
			arc.setCoreWidthOverride(thickness * coreWidthMult);
			arc.setSingleFlickerMode();
